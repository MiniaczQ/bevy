...
use bevy_render::{
    ...
    render_resource::{
        ...
        BindingType, BufferBindingType, BufferCache, BufferDescriptor, BufferUsages, CachedBuffer,
        Extent3d, ShaderStages, ShaderType, StorageTextureAccess, TextureDescriptor,
...
pub fn create_bind_group_layouts(
    ...
    (
        render_device.create_bind_group_layout(
            Some("solari_global_illumination_view_bind_group_layout"),
            &entries[0..entries.len() - 1],
        ),
        render_device.create_bind_group_layout(
            Some("solari_global_illumination_view_with_world_cache_dispatch_bind_group_layout"),
            entries,
        ),
    )
}
...
pub(crate) fn prepare_bind_groups(
    ...
    for (entity, solari_resources, prepass_textures) in &views {
        ...
        let entries = &[
            ...
            entry(c(prepass_textures.depth.as_ref().unwrap())),
            entry(c(prepass_textures.normal.as_ref().unwrap())),
            entry(c(prepass_textures.motion_vectors.as_ref().unwrap())),
        ...
        let bind_groups = SolariGlobalIlluminationBindGroups {
            view_bind_group: render_device.create_bind_group(
                Some("solari_global_illumination_view_bind_group"),
                &pipelines.view_bind_group_layout,
                &entries[0..entries.len() - 1],
            ),
            view_with_world_cache_dispatch_bind_group: render_device.create_bind_group(
                Some("solari_global_illumination_view_with_world_cache_dispatch_bind_group"),
                &pipelines.view_with_world_cache_dispatch_bind_group_layout,
                entries,
            ),
        };
...
fn c(attachment: &ColorAttachment) -> BindingResource<'_> {
    BindingResource::TextureView(&attachment.texture.default_view)
}
